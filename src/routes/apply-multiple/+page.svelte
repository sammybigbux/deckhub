<script lang="ts">
    ...
    import { totalTerms, solvedTerms } from '../../stores/random_store';  // Import stores
    ...
    async function retrieveTermsData(): Promise<void> {
        try {
            ...
            const data = await response.json();
            totalTerms.set(data.totalTerms)  // Reactive updates
            solvedTerms.set(data.solvedTerms);  // Reactive updates
            await tick();
            console.log('Progress should now show:', get(solvedTerms));
        } catch (error) {
    ...
    async function displayRelatedTermResponse(relatedTerm) {
    ...
    async function displayAnswerResponse(userAnswer) {
        // userAnswer is something like "option1", "option2", etc.
    ...
    async function resetTerms(): Promise<void> {
        ...
        if (response.ok) {
            solvedTerms.set(0); // Reset the solvedTerms when terms are reset
        ...
        retrieveTermsData();  // Refresh the terms data after resetting
    }
    ...
    async function passAllTerms(): Promise<void> {
        ...
        if (response.ok) {
            solvedTerms.set($totalTerms); // Mark all terms as solved
    ...
    onMount(async () => {
        ...
        scrollChatBottom();  // Scroll the chat window
...
<div class="flex flex-col min-h-full w-full p-4">
    <div class="w-full mb-4">
        <ChatHeader moduleName="Diagnostic" sectionName="Networking" solvedTerms={5} totalTerms={10} handleActivePlanChange={() => {}} />
    </div>
    <section bind:this={elemChat} class="flex-1 p-4 overflow-y-auto space-y-4">
        {#each messageFeed as bubble}
            {#if bubble.host === false} <!--Render the question (object)-->
                        ...
                        {:else}
                                ...
                                <div class="blue-div btn card-hover">
                                    ...
                                    <div class="flex justify-center flex-wrap gap-2 mt-4">
                                        {#each Object.keys(related_terms) as relatedTerm}
                                            <button class="btn bg-secondary-500 card-hover rounded-container-token" on:click={() => displayRelatedTermResponse(relatedTerm)}>{relatedTerm}</button>
                                        {/each}
                                    </div>
                                </div>
                            {:else if bubble.message.includes('Correct!')}
                                ...
                                <div class="blue-div btn card-hover">
                                    ...
                                    <div class="flex justify-center flex-wrap gap-2 mt-4">
                                        {#each Object.keys(related_terms) as relatedTerm}
                                            <button class="btn bg-secondary-500 card-hover rounded-container-token" on:click={() => displayRelatedTermResponse(curelatedTerm)}>{relatedTerm}</button>
                                        {/each}
                                    </div>
                                </div>
