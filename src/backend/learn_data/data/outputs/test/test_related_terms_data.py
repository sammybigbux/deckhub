...
def test_learn_related_terms_response():
    """
    ...
    if missing_terms:
        ...
        assert 0, f"{length} terms were in {RELATED_TERMS_PATH} but not {RELATED_TERMS_PATH} and have been logged to {LOG_FILE_PATH_RT}"
    ...
    if missing_terms:
        ...
        with open(LOG_FILE_PATH_CORPUS, 'w') as file:
            for term in missing_terms:
                file.write(f"{term}\n")
        assert 0, f"{len(missing_terms)} terms were in {CORPUS_LISTS_PATH} but not {RELATED_TERMS_PATH} and have been logged to {LOG_FILE_PATH_CORPUS}"
