...
def test_related_terms_data():
    ...
    if missing_terms:
        ...
        assert 0, f"{length} missing related terms have been logged to {LOG_FILE_PATH_RT}"
    ...
    if missing_terms:
        ...
        with open(LOG_FILE_PATH_CORPUS, 'w') as file:
            for term in missing_terms:
                file.write(f"{term}\n")
        assert 0, f"{len(missing_terms)} terms in {CORPUS_LISTS_PATH} are not in {RELATED_TERMS_PATH} and have been logged to {LOG_FILE_PATH_CORPUS}"
